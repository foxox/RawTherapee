...
{
    // Notes to remove later: trying to figure out what the various size and position inputs are for
    // imx += 20; // moves left side of image to right, and fills that space with white. invariant to zoom
    // imw /= 5; // invariant to zoom so long as image goes off left side of window boundary
    // startx += 200; // limits how far I can move the crop window to the right side of the image, even with spinbuttons. scales with zoom, so must be image space?

    // cparams is probably the crop settings as specified on the UI??
    // If this is true then let's see... c1 and c2 x and y below must be... 
    // cparams x minus start x  - because startx must be the top left corner of the crop area in the visible portion of the image?
    // Then multiplied by scale to account for zoom
    // yielding c1xyc2xy being in display coordinates? relative to the top left corner of the visible part of the screen
    ...
    const double c2y = (cparams.y + cparams.h - starty) * scale - (fullImageVisible ? 0.0 : 1.0);
    ...
    // If guide drawing is disabled, return early
    if (!drawGuide)
    {
        return;
    }
    ...
    const double recty1 = round(c1y) + imy + 0.5;
    ...
    switch (cparams.guide)
    {
        ...
        // frame is drawn below, after the switch
        break;
        ...
        case rtengine::procparams::CropParams::Guide::GRID:
        {
            ...
            const int w = rectx2 - rectx1, h = recty2 - recty1;
            ...
            if (w > longSideNumLines && h > longSideNumLines) {
                if (w > h) {
                    ...
                    const int shortSideNumLines = (int)round(h * (double)longSideNumLines / w);
                    ...
                    const int shortSideNumLines = (int)round(w * (double)longSideNumLines / h);
        ...
        case rtengine::procparams::CropParams::Guide::RULE_OF_DIAGONALS:
        {
            ...
            const int mindim = std::min(rectx2 - rectx1, recty2 - recty1);
        ...
        case rtengine::procparams::CropParams::Guide::GOLDEN_TRIANGLE_2:
        {
            ...
            const double height = recty2 - recty1;
            const double width = rectx2 - rectx1;
            const double d = sqrt(height * height + width * width);
            const double alpha = asin(width / d);
            const double beta = asin(height / d);
            const double a = sin(beta) * height;
            const double b = sin(alpha) * height;
    ...
    // If any of the cases above set horizontal/vertical guide lines, draw them now:
    // Horizontals
